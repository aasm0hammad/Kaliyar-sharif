      ...
      <main className="flex-1 max-w-[1400px] w-full mx-auto px-4 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
                  ...
                  <li key={cat.id}>
                    <button 
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${
                        activeCategory === cat.id ? 'bg-primary/10 text-primary font-bold' : 'text-gray-600 hover:bg-gray-50'
                      }`}
                    >
                      {cat.name}
                      {activeCategory === cat.id && <CheckCircle2 size={16} className="text-primary" />}
                    </button>
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-gradient-to-br from-secondary to-yellow-600 text-white rounded-xl shadow-sm p-6 relative overflow-hidden">
            ...
            <div className="space-y-6">
              {filteredHotels.map(hotel => (
                <div key={hotel.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden flex flex-col sm:flex-row hover:shadow-lg transition-shadow group relative">
                  ...
                  <div className="w-full sm:w-64 h-48 sm:h-auto overflow-hidden flex-shrink-0 bg-gray-100 relative">
                    <img src={hotel.img_url || hotel.img} alt={hotel.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  </div>
                  ...
                  <div className="flex-1 p-5 lg:p-6 flex flex-col">
                    <div className="flex items-start justify-between gap-4 mb-2">
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="text-xl font-bold text-gray-800 leading-tight group-hover:text-primary transition-colors">{hotel.name}</h3>
                      ...
                      <div className="bg-primary/10 px-2 py-1 rounded-lg flex flex-col items-center flex-shrink-0">
                        <div className="flex items-center gap-1 text-primary font-bold">
                          {hotel.rating} <Star size={14} className="fill-primary" />
                        </div>
                        <span className="text-[10px] text-primary">{hotel.reviews} reviews</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <button className="hidden lg:flex items-center gap-1 text-sm font-bold text-gray-600 hover:text-primary transition-colors">
                          <Info size={16} /> Details
                        </button>
                        <button className="bg-primary hover:bg-primary-light text-white px-6 py-2.5 rounded-lg font-bold text-sm shadow flex items-center gap-2 transition-colors">
