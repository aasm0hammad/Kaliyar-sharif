...
export default function HeroSection() {
  ...
  return (
    <section className="max-w-[1400px] mx-auto px-4 lg:px-8 mt-4 mb-8">
      ...
      <div className="relative rounded-2xl overflow-hidden h-[360px] lg:h-[420px] shadow-lg mb-6 group">
        ...
        <div className={`relative z-[3] h-full flex flex-col justify-center px-6 md:px-12 lg:px-16 w-full md:w-[75%] lg:w-[60%] text-white ${lang === 'ur' ? 'items-end text-right ml-auto' : ''}`}>
          ...
          <button className="bg-white/10 hover:bg-white/20 border border-white/30 text-white px-5 py-2.5 rounded-md font-semibold flex items-center gap-2 w-max transition-colors text-xs md:text-sm">
      ...
      {/* Status Card (Positioned below the hero image) */}
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pb-6 border-b border-gray-100">
          ...
          <div className="flex items-start gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors">
            ...
            <div>
              ...
              <a href="#" className="text-primary text-xs font-semibold hover:underline">
                {lang === 'hi' ? 'अभी देखें' : lang === 'ur' ? 'ابھی دیکھیں' : 'Watch Now'} →
              </a>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors">
            ...
            <div>
              ...
              <div className="text-base font-bold text-gray-800 mb-1">{status.weather.temp}</div>
              <div className="text-xs text-gray-500">
                {status.weather.desc} • {lang === 'hi' ? 'आर्द्रता' : lang === 'ur' ? 'نمی' : 'Humidity'}: {status.weather.humidity}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors">
            ...
            <div>
              ...
              <a href="#" className="text-primary text-xs font-semibold hover:underline">
                {lang === 'hi' ? 'सभी समय देखें' : lang === 'ur' ? 'تمام اوقات دیکھیں' : 'View All Timings'} →
              </a>
            </div>
          </div>
        ...
        <div className="mt-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="bg-primary text-white text-[10px] font-bold px-3 py-1.5 rounded flex items-center gap-1.5 flex-shrink-0">
              <Megaphone size={14} />
              {lang === 'hi' ? 'महत्वपूर्ण घोषणाएं' : lang === 'ur' ? 'اہم اعلانات' : 'ANNOUNCEMENTS'}
            </div>
            <div className="flex-1 overflow-hidden text-xs font-semibold text-gray-700 whitespace-nowrap scroll-smooth">
