    ...
    <div className="flex flex-col min-h-screen bg-gray-50/50">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div className="space-y-2">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <span 
                        className="flex items-center text-xs font-black text-white bg-emerald-600 px-2 py-0.5 rounded shrink-0 cursor-pointer hover:bg-emerald-700 transition-colors"
                        onClick={() => openReviewModal(item)}
                      >
                        {parseFloat(item.rating || 0).toFixed(1)} <Star size={10} className="ml-1 fill-white text-white" />
                      </span>
                    </div>
      ...
      {selectedItem && (
        <ReviewModal 
          ...
          onReviewAdded={() => {
            api.get('/food').then(res => setAllItems(res.data));
          }}
        />
      )}
    </div>
